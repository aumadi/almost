-- =============================================================================
-- Almost App — Banned-viewer blackout (mid-session enforcement)
-- Generated: 2026-05-19
-- =============================================================================
-- Bans rely on auth.users.banned_until to prevent fresh logins, but a
-- user already mid-session keeps their session valid until next sign-in.
-- This migration closes the asymmetry at the data layer so that the
-- first time a banned user refreshes ANY screen after being banned,
-- they immediately see nothing about other users:
--
--   • Crossed Paths returns zero rows.
--   • Chat list returns zero rows.
--   • Chat detail returns zero rows.
--   • Notifications return only own-account events
--     (trip_starts_tomorrow, admin_warning) — anything tied to another
--     user disappears.
--   • Trips screen still returns their own trip rows (own data), but
--     the overlap_users_preview avatar strip is empty.
--   • Reading other people's profiles via direct id returns nothing.
--   • Reading messages directly returns nothing.
--
-- The banned user CAN still read their own profile row so the profile
-- screen doesn't render empty — that's the only carve-out.
--
-- Approach: every check is an inline NOT EXISTS against
--   profiles WHERE id = auth.uid() AND admin_banned_at IS NOT NULL
-- which works under security_invoker views because the updated
-- profiles_select_all policy below always lets a user read their own
-- row regardless of ban status. No SECURITY DEFINER helper needed.
-- =============================================================================
...
-- =============================================================================
-- 1. profiles_select_all RLS — allow self-read always; deny other-reads when viewer is banned
-- =============================================================================
...
CREATE POLICY profiles_select_all
  ...
  USING (
    -- Always allow reading own row, even when banned (profile screen stays functional).
    id = auth.uid()
    OR (
      -- Other people: existing gates + banned viewer cannot read other profiles.
      deleted_at      IS NULL
      ...
      AND NOT EXISTS (
        SELECT 1 FROM public.profiles viewer
         WHERE viewer.id = auth.uid()
           AND viewer.admin_banned_at IS NOT NULL
      )
    )
  );
...
-- =============================================================================
-- 2. messages_select_participant RLS — banned viewer cannot read any messages
-- =============================================================================
...
-- =============================================================================
-- 3. visible_crossed_paths — empty for banned viewers
-- =============================================================================
-- One added top-level WHERE clause. View body otherwise unchanged from
-- 20260429000002.
...
WHERE (cr_latest.status IS DISTINCT FROM 'declined'
  ...
  -- NEW: banned viewer sees zero crossed paths
  AND NOT EXISTS (
    SELECT 1 FROM public.profiles viewer
     WHERE viewer.id = auth.uid()
       AND viewer.admin_banned_at IS NOT NULL
...
-- =============================================================================
-- 4. visible_my_trips — own trips still visible, but preview empty for banned viewer
-- =============================================================================
-- The banned-viewer check goes INSIDE the visible_other_users CTE so
-- the trip rows themselves still return (own data) but the preview
-- jsonb_agg returns []. overlap_count comes out as 0 naturally.
...
LEFT JOIN LATERAL (
  ...
  visible_other_users AS (
    ...
    -- NEW: banned viewer sees no other-user previews
    AND NOT EXISTS (
      SELECT 1 FROM public.profiles viewer
      WHERE viewer.id = auth.uid()
        AND viewer.admin_banned_at IS NOT NULL
...
-- =============================================================================
-- 5. visible_my_chats — empty for banned viewer
-- =============================================================================
...
WITH resolved AS (
  ...
  WHERE chat.deleted_at IS NULL
    ...
    -- NEW: banned viewer sees no chats
    AND NOT EXISTS (
      SELECT 1 FROM public.profiles viewer
       WHERE viewer.id = auth.uid()
         AND viewer.admin_banned_at IS NOT NULL
...
-- =============================================================================
-- 6. visible_chat_detail — empty for banned viewer
-- =============================================================================
...
WITH resolved AS (
  ...
  WHERE chat.deleted_at IS NULL
    ...
    -- NEW: banned viewer sees no chat detail
    AND NOT EXISTS (
      SELECT 1 FROM public.profiles viewer
       WHERE viewer.id = auth.uid()
         AND viewer.admin_banned_at IS NOT NULL
...
-- =============================================================================
-- 7. visible_notifications — banned viewer sees only own-account events
-- =============================================================================
-- Notifications with related_user_id IS NULL (trip_starts_tomorrow,
-- admin_warning) still render. Anything tied to another user
-- (connection_request_received, connection_accepted, new_message)
-- disappears for banned viewers.
...
WHERE notification.deleted_at             IS NULL
  ...
  -- NEW: banned viewer sees only own-account notifications (related_user_id IS NULL)
  AND (
    notification.related_user_id IS NULL
    OR NOT EXISTS (
      SELECT 1 FROM public.profiles viewer
