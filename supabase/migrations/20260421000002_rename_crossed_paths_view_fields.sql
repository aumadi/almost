-- =============================================================================
-- Almost App — Rename visible_crossed_paths fields
-- Generated: 2026-04-21
-- =============================================================================
-- Renames 5 fields on visible_crossed_paths for clarity:
--   my_trip_id        -> overlap_trip_id
--   my_airport_id     -> overlap_airport_id
--   my_airport_iata   -> overlap_airport_iata
--   my_airport_city   -> overlap_airport_city
--   my_slot_type      -> overlap_type
--
-- CREATE OR REPLACE VIEW cannot rename columns, so we DROP the view first
-- and re-create it with the new names. This migration is idempotent:
-- safe to run whether the view previously existed with my_* names, with
-- overlap_* names, or not at all.
-- =============================================================================
...
-- Gender identity label from app_settings JSONB
LEFT JOIN LATERAL (
  SELECT opt->>'label' AS label
    FROM public.app_settings s,
         jsonb_array_elements(s.value) opt
...
-- Picks one row with priority order: accepted > pending > declined.
LEFT JOIN LATERAL (
...
-- RLS of underlying tables runs with the caller's permissions, not the owner.
